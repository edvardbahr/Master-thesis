...
THREE_PARAM_DIR = HERE.parent / "three_param_estimates"

# Reuse the established three-parameter simulator and model definition while
# keeping this live-training workflow in five_param_estimates.
if str(THREE_PARAM_DIR) not in sys.path:
    sys.path.insert(0, str(THREE_PARAM_DIR))
...
def run_hyperparameter_search(
    ...
    for trial_index, values in enumerate(combinations, start=1):
        ...
        trial_config["checkpoint_path"] = str(
            output_dir / f"trial_{trial_index:03d}.pt"
        )
...
def main():
    train_live_summary_nn(
        ...
        hidden_dims_shared_trunk=(128, 64),
        hidden_dims_head=(32, 32),
        checkpoint_path="sv_posterior_summary_nn_live_finance.pt",
        seed=2,
        batch_size=1024,
        n_batches=10,
        val_size=20_000,
